...
状态：顺序多 Agent 运行时设计；清单与 API 见下级契约，开发参考见游戏卡运行文档。
...
新 Session 的启动顺序为共享 State → 按声明顺序执行全部 Agent init → 可选 onStart(ctx) → 等待输入。
onStart 与 onInput 使用相同的 State/Agent/reader 能力，可从初始化后的 Messages 读取开场并 present，无需调用模型。启动整体提交或回滚，完成前不接受输入或保存；读档恢复 started 标记，不重复 init/onStart。
...
一轮成功后才成为新的可保存结果；失败停止并保留已发布的数据和演出现场，禁止保存；只有 retry 才恢复轮前基准并重跑。
