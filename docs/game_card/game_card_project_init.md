...
客户端原生入口已实现，使用 [随客户端安装的开发包](./game_card_devkit.md)。无需 Node、npm、Shell/PowerShell 启动脚本、独立 CLI 安装包、平台源码或网络。
...
初始化与 dry-run、导入安装及实际游玩相互独立。目前 `.wcs/` 在玩家包导入/导出中的统一过滤仍待实现，发布前按开发指南整理独立的纯卡内容目录。
...
测试覆盖最小卡和世界书卡经现有加载器/Schema 校验、UUID、所选库及文档一致性、已有卡和修改保留、目录移动、冲突、未知库、缺失资源、链接、并发文件出现、失败回滚，以及真实客户端在仓库外、PATH 为空时的 JSON 输出、退出码和重复初始化。
