...
<!-- devkit:omit:start -->
适用任务：修改消息模型、规则阶段或执行顺序。  
相关代码：`src/shared/game-card/`、`src/renderer/gameCard/`。  
前置文档：无；从任务路由按需进入。
<!-- devkit:omit:end -->
...
- [操作](./rules/actions.md)
- [Predicate](./rules/predicates.md)
- [Content 描述符与 find](./rules/content.md)
...
```json
{"rules":[{"when":{"phase":"pre_send"},"then":["...操作列表..."]}]}
```
...
```json
{"when":{"phase":"init","length":0},"then":[{"type":"insert","role":"system","content":"{{file:intro}}","_meta":{"source":"game_card_init","visibility":"user_visible"}}]}
```
