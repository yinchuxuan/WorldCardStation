...
<!-- devkit:omit:start -->
适用任务：修改原生卡包导入、导出及安装。  
相关代码：`src/tauri/src/game_card_package.rs`、`src/tauri/src/game_card_imports.rs`。  
...
本文定义平台原生容器。酒馆 V2/V3 复用“导入卡片”入口，自动转换成普通游戏卡目录，进入共用校验和安装管线；仅兼容差异或显式覆盖需要确认，当前范围见 [酒馆卡导入与转换](../../compatibility/tavern/import.md)。下述原生 PNG/ZIP 规则不直接用于解释酒馆 payload。
