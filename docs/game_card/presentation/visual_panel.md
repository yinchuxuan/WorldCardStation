...
背景资源表只接受资源路径，不支持内嵌 `textPanel` 默认值。面板位置由 state 控制，并在背景更新请求发布时应用；只写入 state 不代表立即更新展示。需要立即发布时调用 `visual.updateBackground`，见 [演出操作](../rules/actions.md#presentation-actions)。
...
`game-card-visual-panel` 是阅读背景遮罩，不是正文容器。`game-card-visual-position-*` 按已发布的 `visual.textPanel` 切换。正文样式可使用 [UI runtime](./ui_runtime.md) 中的稳定样式接口。
...
卡片可通过 `display.segmentedReading` 开启分段阅读，支持跨回复回看及 session 阅读位置恢复；分隔符、阅读事件及 patch 提交边界见 [Display](./display.md#分段阅读)。
