...
<!-- devkit:omit:start -->
适用任务：修改消息操作、变量操作或 exec 契约。  
相关代码：`src/shared/game-card/engine/`、`src/shared/game-card/exec/`。  
...
```json
{"type":"insert","predicate":{"index":0},"anchor":"before","role":"system","content":"{{file:worldbook.rules}}","ttl":-1}
```
...
Predicate 的字段、组合和 `when.last.num` 语义见 [Predicate 文档](./predicates.md)。
