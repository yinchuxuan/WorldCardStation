...
<!-- devkit:omit:start -->
适用任务：修改内容引用、模板、find 或文件授权语义。  
相关代码：`src/shared/game-card/content/`。  
...
```json
{"files":{"plot_guides":"chapters/chapter-1/plot_guides.md","worldbook":{"directory":"worldbook","include":["config.json","entries/*.md"]}}}
```
