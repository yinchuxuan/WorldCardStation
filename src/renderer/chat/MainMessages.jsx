...
function MainMessages({ messages, reading, display, displayRevision, isLoading, handleRetry }) {
  ...
  const shown = page && (segmented || reading.ui?.atLatest === false) ? [{ ...page.record, content: page.text }] : messages;
  return <div className="chat-messages-layer" data-gc-part="message-surface">
    {shown.map((msg, index) => <MessageRow key={msg.id} message={msg} retryButton={Renderers.renderRetryBtn({
