...
export default function useChatDisplay(config, state, messages, isLoading, statePatchEnabled = true) {
  const display = React.useMemo(() => resolveDisplayState(config, state, statePatchEnabled), [config, state, statePatchEnabled]);
