...
export async function startModelServer(port) {
  ...
  const server = createServer(async (request, response) => {
    ...
    const text = url.pathname.startsWith('/multi-agent/') ? (config.messages?.some(msg => msg.content.startsWith('Judge:'))
      ? '<state_patch>{"verdict":"通过"}</state_patch>' : '实际展示')
      : url.pathname.startsWith('/reader-judge/') ? '辅助裁定，不应展示。'
